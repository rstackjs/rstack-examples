...
async function ServerDataCard({ delay }: { delay: number }) {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md border border-gray-200">
      ...
      <div className="text-xs text-gray-500 mt-1">Computed at: {data.computedAt}</div>
  ...
  const stream = await renderToReadableStream(
    <div className="space-y-4">
      <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
        ...
        <p className="text-amber-700 text-sm mt-1">This entire tree is rendered on the server</p>
      </div>
