  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-2xl font-bold text-gray-900 min-w-[60px] text-center">{counter}</div>
        <button
      ...
      <div className="flex gap-4 text-sm">
        ...
        <div className="px-3 py-1 bg-gray-100 rounded-full">
          ...
          <span className="font-bold text-gray-700">{(cacheStats.size / 1024).toFixed(2)} KB</span>
        </div>
      </div>
        ...
        source && (
          <div
            className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg ${
              source === 'cache' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
            }`}
          >
            <div
            ...
            <span className="font-medium">
              {source === 'cache' ? 'Loaded from Dexie cache' : 'Fetched from server'}
            </span>
            {loadTime !== null && (
              <span className="text-xs opacity-75">({loadTime.toFixed(1)}ms)</span>
            )}
          </div>
        )
      )}
      ...
      {result && (
        <div className="border-2 border-dashed border-cyan-300 rounded-lg p-2">
          <div className="text-xs text-cyan-600 mb-2 font-medium">RSC for Counter #{counter}:</div>
          {result}
        </div>
      )}
      ...
      <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
        <strong>How it works:</strong> When you change the counter, the app automatically checks
        Dexie (IndexedDB) for a cached Flight payload. If found, it decodes the cached string
        directly. If not found, it fetches from the server, stores the payload in Dexie, then
        decodes it. Each counter value gets its own cache entry with unique server-generated data.
      </div>
    </div>
  );
}
