...
export function DirectFlightStreamDemo() {
  ...
  return (
    <div className="space-y-4">
      ...
      {streamInfo && (
        <div className="text-sm text-gray-600 font-mono bg-gray-100 p-2 rounded">{streamInfo}</div>
      )}

      {result && (
        <div className="border-2 border-dashed border-purple-300 rounded-lg p-2">
          <div className="text-xs text-purple-600 mb-2 font-medium">Server-Rendered Content:</div>
          {result}
        </div>
      )}
    </div>
