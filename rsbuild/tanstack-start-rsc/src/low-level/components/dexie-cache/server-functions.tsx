...
function CounterCard({ count }: { count: number }) {
  ...
  return (
    <div className={`p-6 bg-linear-to-r ${color.bg} rounded-lg text-white shadow-lg`}>
      ...
      <div className={`${color.text} text-xs mt-4 font-mono`}>Generated: {timestamp}</div>
    </div>
  );
}
...
export const getCounterRscPayload = createServerFn()
  ...
  .handler(async ({ data }) => {
    ...
    const stream = await renderToReadableStream(<CounterCard count={data.count} />);
