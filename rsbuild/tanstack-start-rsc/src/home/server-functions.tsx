...
const demos: Array<DemoInfo> = [
  ...
  },
  {
    ...
    path: '/pokemon-rsc',
    features: ['Async server components', 'Server-side data fetching', 'renderServerComponent API'],
...
export const getHomePage = createServerFn().handler(async () => {
  ...
  const src = await createCompositeComponent(
    (props: { renderDemoCard?: (data: { demo: DemoInfo }) => React.ReactNode }) => (
      <div className="py-10">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Explore different patterns and APIs for building applications with React Server
              Components in TanStack Start.
            </p>
          </div>
          ...
          <div className="mt-12 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="font-bold text-gray-900 mb-3">About These Examples</h3>
            <p className="text-gray-600 text-sm">
              These examples demonstrate various React Server Component patterns available in
              TanStack Start. Server components are rendered on the server and sent to the client as
              a Flight stream. Client components handle interactivity and state management. The
              visual indicators in the footer show which parts are server-rendered (blue) vs
              client-interactive (green).
            </p>
          </div>
        </div>
      </div>
